...
public class PrescriptionGenerator
{
    ...
    public static void CreatePrescription()
    {
        ...
        string filepath = Path.Combine(desktopPath, "Prescription_Sample.pdf");
        ...
        try
        {
            ...
            string patientName = "Zayan Ahmed";
            string patientAge = "34";
            string patientGender = "Male";
            string patientWeight = "78 kg";
            string diagnosis = "Acute Viral Pharyngitis";
            string prescriptionDate = DateTime.Now.ToString("dd-MMM-yyyy");
            ...
            patientInfoTable.SpacingBefore = 10f; // Add space after the header.
            ...
            patientInfoTable.AddCell(CreateValueCell(patientGender));

            PdfPCell weightLabelCell = CreateLabelCell("Weight:");
            PdfPCell weightValueCell = CreateValueCell(patientWeight);
            weightValueCell.Colspan = 3;
            patientInfoTable.AddCell(weightLabelCell);
            patientInfoTable.AddCell(weightValueCell);
            ...
            // --- 3. ADD MAIN CONTENT AREA & SIGNATURE ---
            ...
            Font fontMedication = FontFactory.GetFont(FontFactory.HELVETICA, 10, BaseColor.BLACK);
            ...
            contentCell.AddElement(new Paragraph("1. Tab. Napa Extend (Paracetamol 665mg)", fontMedication) { IndentationLeft = 20f });
            contentCell.AddElement(new Paragraph("     1 + 0 + 1 (after meal) - 5 days", fontMedication) { IndentationLeft = 25f });
            contentCell.AddElement(new Paragraph(" ")); // Spacer
            contentCell.AddElement(new Paragraph("2. Tab. Fexo (Fexofenadine 120mg)", fontMedication) { IndentationLeft = 20f });
            contentCell.AddElement(new Paragraph("     0 + 0 + 1 - 7 days", fontMedication) { IndentationLeft = 25f });
            contentCell.AddElement(new Paragraph(" ")); // Spacer
            contentCell.AddElement(new Paragraph("3. Adryl Lozenges (for sucking)", fontMedication) { IndentationLeft = 20f });
            contentCell.AddElement(new Paragraph("     1 lozenge 4 times daily", fontMedication) { IndentationLeft = 25f });
            contentCell.AddElement(new Paragraph(" ") { SpacingBefore = 20f }); // Spacer
            ...
            adviceList.Add(new ListItem(" Gargle with lukewarm salt water 3-4 times a day.", fontMedication));
            adviceList.Add(new ListItem(" Drink plenty of warm fluids.", fontMedication));
            adviceList.Add(new ListItem(" Take complete rest.", fontMedication));
            ...
            PdfPCell signatureCell = new PdfPCell
            {
                ...
                MinimumHeight = 150f // Ensures space between prescription and signature
            };
    ...
    // Overload to create an empty, underlined cell
    private static PdfPCell CreateValueCell()
    {
        return new PdfPCell { Border = Rectangle.BOTTOM_BORDER, BorderColor = BaseColor.LIGHT_GRAY, BorderWidthBottom = 1f, PaddingBottom = 10 };
    }

    // Overload to create a cell with text content
...
public class PrescriptionPageEvents : PdfPageEventHelper
{
    ...
    public override void OnEndPage(PdfWriter writer, Document document)
    {
        ...
        // Cell 1: Doctor's Name and Qualification
        ...
        textCell.AddElement(new Phrase("Dr. Aris Thorne", fontHeaderLarge));
        textCell.AddElement(new Phrase("M.D., F.A.C.P. (Internal Medicine)", fontHeaderSmall));
        headerTable.AddCell(textCell);

        // Cell 2: Stethoscope Icon
        ...
        PdfPCell clinicCell = new PdfPCell(new Phrase("Gopalganj Central Clinic", fontFooter))
        ...
        PdfPCell addressCell = new PdfPCell(new Phrase("📍 Central Road, Gopalganj, Dhaka", fontFooter))
        ...
        PdfPCell phoneCell = new PdfPCell(new Phrase("📞 +880-171-234-5678", fontFooter))
